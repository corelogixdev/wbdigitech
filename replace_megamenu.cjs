...
let replaceEndStr = endStr;

if (endIndex === -1) {
    endIndex = content.indexOf(endStr2);
    replaceEndStr = endStr2;
}

if (startIndex !== -1 && endIndex !== -1) {
    let newContent = content.substring(0, startIndex) + newMegaMenu + '\n                                    {{-- AI Marketing Simple Dropdown --}}' + content.substring(endIndex + replaceEndStr.length);
